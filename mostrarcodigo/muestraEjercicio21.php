<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Véro Grué - ProyectoTema3 muestraEjercicio21</title>
    <link rel="stylesheet" href="../webroot/css/styleEjercicios.css">
</head>
<body>
    <header class="header">
        <h1>Muestra Ejercicio 21</h1>
    </header>
    <main>
        <section>
            
            <?php 
            echo"<h3>Contenido de la pagina ejercicio 21</h3>";
     highlight_file('../codigoPHP/ejercicio21.php');
     
     echo"<h3><br><br>Contenido de la pagina Tratamiento.php</h3>";
     highlight_file('../codigoPHP/Tratamiento.php');
            
    ?>
        </section>
        
    
    </main>
    <footer class="footer">
      <div class="footerContent">
            <div><p class="copyright">
            2025-26 IES LOS SAUCES. &#169;Todos los derechos reservados.</p> <address><a href="../indexProyectoTema3.php">Véronique Grué.</a> Fecha de Actualización :
                <time datetime="2025-10-02"></time> 02-10-2025 </address>
          </div>
            
          </div>
          
    </footer>
    
</body>
</html>

